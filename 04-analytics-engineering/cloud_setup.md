...
```
-- /* create external table from "gs://zoomcamp_bucket_kestra-sandbox-8656/green*.csv.gz" */
CREATE EXTERNAL TABLE
  `kestra-sandbox-8656`.`nytaxi`.`green_tripdata_external_from_gcs`( VendorID STRING,
    ...
    payment_type INT64,
    trip_type STRING,
    congestion_surcharge NUMERIC) OPTIONS ( format = 'CSV',
    uris = ['gs://zoomcamp_bucket_kestra-sandbox-8656/green*.csv.gz']);

select count(1) from `kestra-sandbox-8656.nytaxi.green_tripdata_external_from_gcs`;
```

```
--GREEN

CREATE EXTERNAL TABLE `kestra-sandbox-8656`.`nytaxi`.`green_tripdata_external`
  OPTIONS (
    format = 'CSV',
    uris = ['gs://zoomcamp_bucket_kestra-sandbox-8656/green*.csv.gz'],
    compression = 'GZIP',
...
select count(1) from nytaxi.green_tripdata_external;
--7778101

CREATE OR REPLACE TABLE `kestra-sandbox-8656`.`nytaxi`.`green_tripdata` AS SELECT * FROM `kestra-sandbox-8656`.`nytaxi`.`green_tripdata_external`;

--YELLOW

CREATE EXTERNAL TABLE `kestra-sandbox-8656`.`nytaxi`.`yellow_tripdata_external`
  OPTIONS (
    format = 'CSV',
    uris = ['gs://zoomcamp_bucket_kestra-sandbox-8656/yellow*.csv.gz'],
    compression = 'GZIP',
    skip_leading_rows = 1,
    allow_quoted_newlines = TRUE,
    field_delimiter = ',');

select count(1) from nytaxi.yellow_tripdata_external;
--109047518

CREATE OR REPLACE TABLE `kestra-sandbox-8656`.`nytaxi`.`yellow_tripdata` AS SELECT * FROM `kestra-sandbox-8656`.`nytaxi`.`yellow_tripdata_external`;
